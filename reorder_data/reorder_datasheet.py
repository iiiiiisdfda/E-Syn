...
input_csv_path = os.path.join(parent_dir, 'res_data_rc64b.csv')
output_csv_delay_path = os.path.join(parent_dir, 'sorted_delay.csv')
output_csv_area_path = os.path.join(parent_dir, 'sorted_area.csv')
...
data = [[row[0], float(row[1]), float(row[2]), float(row[3]), float(row[4])] for row in data]
