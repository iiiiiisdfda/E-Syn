...
def main():
    ...
    # 提取特征和目标
    # 只使用原本的 E-Syn 特征（原始特征）
    # 原始特征包括：ASTSize, ASTDepth, 运算符计数 (+, !, *, &), SUM_LIB, SUM_NODE, AVE_LIB
    original_features = [
        'ASTSize', 'ASTDepth', '+', '!', '*', '&', 'SUM_LIB', 'SUM_NODE', 'AVE_LIB'
    ]
    ...
    if missing_features:
        print(f"⚠️  Warning: Some original features are missing: {missing_features}")
        print(f"   Available original features: {available_original_features}")
    ...
    feature_cols = available_original_features
    
    # 打印特征信息
    print(f"\n✓ Using original E-Syn features only: {len(feature_cols)} features")
    ...
    # 创建输出文件夹（使用 xgb_data_0 存储原始特征模型）
    output_dir = f'xgb_data_0'
